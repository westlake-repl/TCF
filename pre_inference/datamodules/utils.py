...
def pre_inference(
    ...
    final_n_freeze = n_hidden_layers - last_n_unfreeze
    ...
    if layer_wise:
        ...
        for n_freeze in range(start, end):
            ...
            if return_code == 0:
                ...
                if n_hidden_layers - n_freeze >= 2 and prev_embs_file != "none":
...
class InferenceFileProcessor:
    ...
    def __init__(self,
        ...
        if self.plm_config.model_type in ["bert", "opt"]:
            self.n_hidden_layers = self.plm_config.num_hidden_layers
        elif self.plm_config.model_type in ["t5"]:
            self.n_hidden_layers = self.plm_config.num_layers
        else:
            raise NotImplementedError(f"{self.plm_config.model_type} not implemented")
        
        assert self.n_hidden_layers >= last_n_unfreeze
        assert last_n_unfreeze > -1
        if last_n_unfreeze == self.n_hidden_layers:
        ...
        self.n_freeze = self.n_hidden_layers - last_n_unfreeze
