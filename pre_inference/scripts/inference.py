...
from models.partial_bert import PartialBertModel
from models.partial_t5 import PartialT5EncoderModel
...
class FrozenPartialPLM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if self.plm_config.model_type == "opt":
            self.plm = PartialOPTModel.from_pretrained(
                plm_name,
                keep_embed_layer=keep_embed_layer,
                keep_decoders_range=keep_inner_layers_range)

        elif self.plm_config.model_type == "bert":
            self.plm = PartialBertModel.from_pretrained(
                plm_name,
                keep_embed_layer=keep_embed_layer,
                keep_encoders_range=keep_inner_layers_range,
                add_pooling_layer=False)
            
        elif self.plm_config.model_type == "t5":
            self.plm = PartialT5EncoderModel.from_pretrained(
                plm_name,
                keep_embed_layer=keep_embed_layer,
                keep_hidden_layers_range=keep_inner_layers_range)
            
        else:
            raise ValueError(f"Model type {self.plm_name.plm_config.model_type} is not supported")
...
if __name__ == "__main__":
    ...
    if plm_config.model_type in ["bert", "opt"]:
        n_hidden_layers = plm_config.num_hidden_layers
    elif plm_config.model_type in ["t5"]:
        n_hidden_layers = plm_config.num_layers
    else:
        raise NotImplementedError(f"{plm_config.model_type} not implemented")
        
    levels = ["none", "emb"] + [i for i in range(n_hidden_layers)]
    ...
    last_n_unfreeze = n_hidden_layers - n_freeze
    ...
    if target_layer is None:
        ...
        keep_inner_layers_range = (0, target_layer+1)
    else:
        keep_inner_layers_range = (input_layer+1, target_layer+1)
